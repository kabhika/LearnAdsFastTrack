...
const DEFAULT_STATE = {
  ...
  settings: {
    ...
    startTime: "21:00",
    perDay: 6                             // sprint pace: stations per service day
  }
};
...
function buildAssignments() {
  ...
  const perDay = Math.max(1, state.settings.perDay || 6);
  const byId = {};      // taskId -> dateISO
  const byDate = {};    // dateISO -> [tasks]
  let cursor = today();
  if (!isActive(cursor) || completedToday().length >= perDay) {
  ...
  for (const t of pending) {
    ...
    byId[t.id] = dISO;
    (byDate[dISO] = byDate[dISO] || []).push(t);
    if (++slot >= perDay) { slot = 0; cursor = nextActive(addDays(cursor, 1)); }
  }
...
const LINE_INK = {
  ...
  p5: "var(--magenta)", p6: "var(--sky)", p7: "var(--teal)"
};
const LINE_NAME = {
  p1: "Scarlet Line", p2: "Cobalt Line", p3: "Amber Line", p4: "Green Line",
  p5: "Magenta Line", p6: "Sky Line", p7: "Teal Line"
};
...
const TYPE_LABEL = {
  ...
  project: "Project", article: "Article", book: "Book", tool: "Tool",
  practice: "Practice"
};
...
function renderStats() {
  ...
  $("#stat-hours").textContent = `${Math.round((total - done) * MIN_PER_STATION / 60)}h`;
...
function renderToday() {
  ...
  if (pending.length === 0) {
    html += `<div class="hero-task" style="--line: var(--green)"><span class="terminus-dot"></span>
      <h3>Terminus reached. You run paid traffic on Google and Meta.</h3></div>`;
  } else {
...
function renderMap() {
  ...
  const terminus = `
    ...
    <text class="map-label strong" x="${start.x - 14}" y="${start.y + 30}">Depart: never run an ad</text>
    <circle cx="${end.x}" cy="${end.y}" r="9" fill="var(--porcelain)"></circle>
    <circle cx="${end.x}" cy="${end.y}" r="4" fill="${LINE_INK.p7}"></circle>
    <text class="map-label strong" x="${end.x + 16}" y="${end.y + 28}">Terminus: client campaigns live</text>`;
...
function renderTimeline() {
  ...
  for (const p of PHASES) {
    if (p.term === "short" && !shortShown) {
      html += `<div class="term-divider"><span>The sprint</span> first client campaign live, tracked, reported</div>`;
      shortShown = true;
    }
    if (p.term === "long" && !longShown) {
      html += `<div class="term-divider long"><span>After the sprint</span> keep momentum, go deeper</div>`;
      longShown = true;
    }
    const tasks = TASKS.filter(t => t.phase === p.id);
    const done = tasks.filter(t => state.completions[t.id]).length;
    const pct = Math.round((done / tasks.length) * 100);
    const days = Math.ceil(tasks.length / Math.max(1, state.settings.perDay || 6));
    ...
    html += `
      <div class="legend-row ${done === tasks.length ? "done-line" : ""}">
        ...
        <div class="legend-stats">
          ...
          ${done}/${tasks.length} &middot; ~${days} d<br>${dateRange}
        </div>
      </div>`;
  }
  el.innerHTML = html;
...
function renderSchedule() {
  ...
  (assignments.byDate[dISO] || []).forEach(task => {
  ...
  const futureDates = Object.keys(assignments.byDate).filter(dd => dd > dISO).sort().slice(0, 14);
  for (const dd of futureDates) {
    const dayTasks = assignments.byDate[dd];
    rows.push(rowHtml(fromISO(dd),
      dayTasks.map(task => `<span class="row-future">${task.title}</span><span class="row-phase" style="--line:${LINE_INK[task.phase]}">${LINE_NAME[task.phase]}</span>`).join("<br>"),
      "future", fmtTimeSlot()));
  }
  if (Object.keys(assignments.byDate).length > 15) {
    rows.push(`<div class="sched-more">+ ${Object.keys(assignments.byDate).length - 15} more departures after this. They shift automatically as you ride.</div>`);
  }
...
function renderSettings() {
  ...
  const paceWrap = $("#pace-picker");
  if (paceWrap) {
    paceWrap.innerHTML = [2, 4, 6, 8].map(n =>
      `<button class="day-chip ${state.settings.perDay === n ? "on" : ""}" data-pace="${n}">${n}/day</button>`
    ).join("");
    paceWrap.querySelectorAll("[data-pace]").forEach(btn => {
      btn.addEventListener("click", () => {
        state.settings.perDay = Number(btn.dataset.pace);
        saveState();
        renderSettings();
        renderAll();
      });
...
function setupChrome() {
  ...
  $("#export-btn").addEventListener("click", () => {
    ...
    a.download = `adsfast-backup-${iso(today())}.json`;
    a.click();
  ...
  const post = () => {
    try {
      ...
      window.parent.postMessage(
        { type: "line-progress", line: "ads-fasttrack", done: Object.keys(state.completions).length, total: TASKS.length, phases },
        "*"
      );
